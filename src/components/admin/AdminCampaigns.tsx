    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <button
          ...
          className="flex items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground hover:opacity-90 transition"
        >
              ...
              <div key={c.id} className="rounded-xl bg-card border border-border p-5 space-y-4 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium ${
                    status === 'Đang chạy' ? 'bg-success/10 text-success' : 'bg-destructive/10 text-destructive'
                  }`}>{status}</span>
                </div>
...
const steps = [
  { id: 1, label: 'Chọn Vendor', icon: <Store className="w-4 h-4" /> },
  { id: 2, label: 'Cấu hình', icon: <Settings className="w-4 h-4" /> },
  { id: 3, label: 'Xác nhận', icon: <Eye className="w-4 h-4" /> },
];
...
function CreateCampaignWizard({ onClose, onCreated }: { onClose: () => void; onCreated: (c: Campaign, qty: number) => void }) {
  ...
  const canNext = () => {
    ...
    if (step === 2) return !!name.trim() && qty > 0 && !!expiry;
    return true;
  };
        ...
        <div className="px-6 py-5 min-h-[280px]">
          ...
          {step === 1 && (
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground mb-4">Chọn đối tác vendor cho chiến dịch</p>
                ...
                {vendors.map((v) => (
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">Đối tác vendor</p>
                    </div>
                    {vendorId === v.id && (
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div>
                ...
                <input value={name} onChange={(e) => setName(e.target.value)} required placeholder="VD: Flash Sale Tháng 4" className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
              <div>
                <label className="block text-sm font-medium text-foreground mb-1">Mô tả</label>
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2} placeholder="Mô tả ngắn về chiến dịch..." className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring resize-none" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" min={1} max={1000} value={qty} onChange={(e) => setQty(Number(e.target.value))} className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Ngày hết hạn <span className="text-destructive">*</span></label>
                  <input type="date" value={expiry} onChange={(e) => setExpiry(e.target.value)} className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-foreground mb-1">Chi tiêu tối thiểu (₫)</label>
                  <input type="number" min={0} value={minSpend} onChange={(e) => setMinSpend(Number(e.target.value))} placeholder="0" className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
                </div>
                {vType === 'Discount %' && (
                  <div>
                    <label className="block text-sm font-medium text-foreground mb-1">Giảm tối đa (₫)</label>
                    <input type="number" min={0} value={maxDiscount} onChange={(e) => setMaxDiscount(Number(e.target.value))} placeholder="0" className="w-full rounded-lg border border-input bg-background px-3 py-2.5 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-ring" />
                  </div>
                )}
              </div>
            </div>
          ...
          {step === 3 && (
            <div className="space-y-5">
              <p className="text-sm text-muted-foreground">Kiểm tra thông tin chiến dịch trước khi tạo</p>
              ...
              <div className="rounded-xl border-2 border-dashed border-primary/30 bg-primary/5 p-5 space-y-3">
                ...
                <div className="rounded-lg bg-card border border-border p-4 flex items-center gap-4">
                  <span className="text-3xl">{selectedVendor.logo}</span>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-foreground">{valueLabel()}</p>
                    <p className="text-xs text-muted-foreground">{selectedVendor.name} · {voucherTypeLabels[vType]}</p>
                    {minSpend > 0 && <p className="text-[10px] text-muted-foreground mt-0.5">Đơn tối thiểu {minSpend.toLocaleString()}₫</p>}
                  </div>
                  <div className="text-right">
                    <span className="rounded-full bg-success/10 text-success px-2 py-0.5 text-[10px] font-medium">Hoạt động</span>
                    <p className="text-[10px] text-muted-foreground mt-1">HSD: {expiry}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-xl border border-border divide-y divide-border">
                ...
                {description && <SummaryRow label="Mô tả" value={description} />}
                ...
                {vType === 'Discount %' && maxDiscount > 0 && <SummaryRow label="Giảm tối đa" value={`${maxDiscount.toLocaleString()}₫`} />}
              </div>
            </div>
          )}
        </div>
          ...
          {step < 3 ? (
            ...
            >
              {saving ? (
                ...
                <>
                  <Check className="w-4 h-4" /> Tạo {qty} voucher
                </>
              )}
            </button>
          )}
